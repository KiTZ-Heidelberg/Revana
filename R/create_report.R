...
create_plots_and_HTML_part_4 <- function(processed_data,
    ...
    part4_html_by_gene <- genes_to_analyze_further %>% purrr::map(function(GENE_NAME) {
        create_report_gene_subdocument(
            processed_data,
            GENE_NAME,
            color_palette,
            HTML_report_subdocument_directory,
...
create_HTML_report_new <- function(HTML_report_output_dir_path, output_paths_file_path, use_cache = F, create_cache = F, max_n_genes_to_analyze_further = 500, additional_genes_to_analyze_further = character(0), skip_creating_toggle_images = F, has_run_tf_binding_site_analysis = F, add_custom_HTML_to_gene_subdocument = NULL, add_custom_HTML_to_gene_subdocument_sample_data = NULL, genome_name = "hg19") {
    # INITIALIZATION #############################################
    ...
    # part 1/2
    create_plots_part_1_and_2(processed_data, HTML_report_figure_directory, skip_creating_toggle_images, color_palette)
